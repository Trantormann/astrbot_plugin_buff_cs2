...
- **武器类**（步枪/手枪/冲锋枪/霰弹枪/机枪/刀/手套）可在插件配置中开启**同品类扫描**（`enable_category_scan`，默认关闭）：扫描同品类 Top 商品，按「低印花溢价率(SP<30%) > 低价格 > 低磨损」排序，推荐最值得关注的三件。开启后每次武器查询会多花约十几秒。
...
3. 其余配置项（均有默认值）：
   ...
   - `enable_category_scan`：是否启用同品类扫描（消息二），默认关闭
